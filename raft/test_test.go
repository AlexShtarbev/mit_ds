...
func TestReElection(t *testing.T) {
	...
	printOut("leader1 = " + strconv.Itoa(leader1))

	// if the leader disconnects, a new one should be elected.
	printOut("disconnect " + strconv.Itoa(leader1))
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	printOut("connect " + strconv.Itoa(leader1))
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	printOut("leader2 = " + strconv.Itoa(leader2))

	// if there's no quorum, no leader should
	// be elected.
	printOut("disconnect " + strconv.Itoa(leader2))
	cfg.disconnect(leader2)
	printOut("disconnect " + strconv.Itoa((leader2 + 1) % servers))
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	printOut("connect " + strconv.Itoa((leader2 + 1) % servers))
